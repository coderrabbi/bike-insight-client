...
const MyProducts = () => {
    ...
    fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/products?email=${user.email}`)
        .then((res) => res.json())
        .then((data) => {
            setProducts(data);
        });
    ...
    const handelDelete = (id) => {
        ...
        if (confirm) {
            fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/products/${id}`, {
                method: 'DELETE',
            })
                .then((res) => res.json())
                .then((data) => {
    ...
    return (
        <div>
            <div className="overflow-x-auto mt-10">
                {products.length < 0 ? (
                    <h1 className="text-center md:text-[60px] text-[30px] ">NO PRODUCTS FOUND</h1>
                ) : (
